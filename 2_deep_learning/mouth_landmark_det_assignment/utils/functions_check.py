...
def draw_landmarks(img, pts, style='fancy', wfp=None, show_flag=False, **kwargs):
    ...
    for i in range(len(pts)):
        if dense_flag:
            ...
            '''
            plt.plot(pts[i][0, 63], pts[i][1, 63], 'xk')
            plt.plot(pts[i][0, 65], pts[i][1, 65], 'xk')

            # mouth height
            distance = sqrt( (pts[i][0, 65] - pts[i][0, 63])**2 +
                              (pts[i][1, 65] - pts[i][1, 63])**2)
            #print (distance)
            '''
...
def cv_draw_landmark(img_ori, pts, box=None, color=RED, size=3):
    ...
    if n <= 106:
        '''
        for i in range(48, 68): # just the lips
